...
export async function updateComment(
  ...
  await db
    ...
    .values({ comment_id: commentId, updated_content: current.content });
...
export async function deleteComment(commentId: number, pathname: string) {
  ...
  await db
    .update(comments)
    .set({ hidden: true, hidden_reason: "RESOLVED" })
    .where(eq(comments.id, commentId));
  revalidatePath(pathname);
}
...
export async function toggleReaction(
  ...
  const comment = await db.query.comments.findFirst({
    where: and(eq(comments.id, commentId), eq(comments.hidden, false))
  });
...
async function persistMentionsAndNotifications(
  ...
  const mentionedUsers = await db
    .select()
    .from(users)
    .where(sql`LOWER(${users.username}) IN ${usernames}`);
